...
class CustomerViewTest(TestCase):
    ...
    def test_admin_view(self):
        """
        Тест проверки возможности просмотра списка активных клиентов
        администратором. Возвращает код 403, после наделения
        необходимыми правами - 200.
        """
        self.login_admin()
        response = self.client.get(reverse("customers:customers-list"))
    ...
    def test_operator_detail(self):
        ...
        авторизованным пользователем. Возвращает код 302, после наделения необходимыми
        правами - 200.
        """
        self.login_operator()
        response = self.client.get(reverse("customers:customers-detail", kwargs={"pk": self.customer.pk}))
    ...
    def test_admin_detail(self):
        """
        Тест проверки возможности просмотра деталей активного клиента
        администратором. Возвращает код 302, после наделения необходимыми
        правами - 200.
        """
        self.login_admin()
        response = self.client.get(reverse("customers:customers-detail", kwargs={"pk": self.customer.pk}))
    ...
    def test_operator_create(self):
        ...
        prod_permission = Permission.objects.get(content_type=product_ct, codename='add_product')
        advert_ct = ContentType.objects.get_for_model(Advert)
        ads_permission = Permission.objects.get(content_type=advert_ct, codename='add_advert')
