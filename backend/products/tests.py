...
class ProductTestView(TestCase):
    ...
    def setUp(self):
        """
        Метод установки значений перед каждым тестом, то есть значения создаются
        перед каждым новым тестом заново.
        """
    ...
    def test_update_product(self):
        ...
        response = self.client.post(
            reverse("products:product-update", kwargs={"pk": self.product.pk}),
            data=data,
        )
        self.assertEqual(response.status_code, 302)
        self.assertIn(str(settings.LOGIN_URL), response.url)
    ...
    def test_update_marketer(self):
        """
        Тест возможности маркетологом создать услугу, при отсутствии прав возвращает
        код 403, при получении разрешений на обновление и просмотр услуг - 302 и
        ...
        self.assertRedirects(
            response, reverse("products:product-detail", kwargs={"pk": self.product.pk})
        )
    ...
    def test_update_admin(self):
        """
        Тест возможности админом создать услугу возвращает код 302 и перенаправляет
        на страницу услуги.
        """

        self.login_admin()
        data = {"name": "ProdUpdate", "cost": 90.00, "description": "qwe1"}
        response = self.client.post(
            reverse("products:product-update", kwargs={"pk": self.product.pk}),
            data=data,
        )
