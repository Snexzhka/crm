...
class LeadsModelsTest(TestCase):
    ...
    def setUpClass(cls):
        """
        Метод установки данных для тестов, срабатывает вначале, перед
...
class LeadsViewTest(TestCase):
    ...
    def tearDownClass(cls):
        """
        Метод для удаления медиа файлов после прогона всех тестов.
        Запускается в конце, после отработки всех тестов.
        """
    ...
    def test_view_leads(self):
        """
        Тест проверки невозможности просмотра списка лидов неавторизованным
    ...
    def test_leads_operator(self):
        """
        Тест проверки возможности просмотра списка лидов оператором.
        Возвращает изначально код 403, после предоставления
        разрешений - 200.
        """
    ...
    def test_detail_leads(self):
        """
        Тест проверяющий невозможность просмотра деталей лида неавторизованным
        пользователем. Возвращает код 302 и перенаправляет на страницу входа.
        """

        self.client.logout()
        response = self.client.get(
            reverse("leads:leads-detail", kwargs={"pk": self.lead.pk}),
        )
        self.assertEqual(response.status_code, 302)
        self.assertIn(str(settings.LOGIN_URL), response.url)
    ...
    def test_operator_detail(self):
        """
        Тест, проверяющий возможность просмотра оператором деталей лидов.
        Изначально дает код 403, после предоставления разрешений 200.
        """
    ...
    def test_admin_detail(self):
        """
        Тест, проверяющий возможность просмотра админом деталей лидов.
        Возвращает код 200.
        """
    ...
    def test_operator_create(self):
        """
        Тест, проверяющий возможность создания лида оператором. Сразу возвращает
        код 403, после предоставления прав - 302 и перенаправляет на список лидов.
        """
        ...
        advert_permission = Permission.objects.get(
            content_type=ads_ct, codename="view_advert"
        )
        product_permission = Permission.objects.get(
            content_type=prod_ct, codename="view_product"
        )
    ...
    def test_update_leads(self):
        ...
        response = self.client.post(
            reverse("leads:leads-update", kwargs={"pk": self.lead.pk}),
            data=data,
        )
        self.assertEqual(response.status_code, 302)
        self.assertIn(str(settings.LOGIN_URL), response.url)
    ...
    def test_operator_update(self):
        """
        Тест, проверяющий возможность обновления лида оператором. Сразу возвращает
        код 403, после предоставления прав - 302 и перенаправляет на детали лида.
        """
    ...
    def test_delete_leads(self):
        ...
        self.client.logout()
        response = self.client.post(
            reverse("leads:leads-delete", kwargs={"pk": self.lead.pk}),
        )
        self.assertEqual(response.status_code, 403)

    def tset_operator_delete(self):
        """
        Тест, проверяющий возможность удаления лида оператором. Сразу возвращает
        код 403, после предоставления прав - 403 (согласно представления удалить
        может только админ).
        """
