...
class ContractModelTest(TestCase):
    ...
    def test_model_contract(self):
        ...
        self.assertEqual(
            str(self.contract.start_date), str(datetime.datetime.now().date())
        )
...
class ContractTestView(TestCase):
    ...
    def tearDownClass(cls):
        """
        Метод для удаления медиа файлов после прогона всех тестов. Запускается
        в конце, после отработки всех тестов.
        """
    ...
    def test_list_view_contract(self):
        """
        Тест проверки невозможности просмотра списка контрактов неавторизованным
    ...
    def test_list_contract_manager(self):
        """
        Тест проверки возможности просмотра списка контрактов авторизованным
        пользователем. Возвращает код 403 - нет доступа. После предоставления
        необходимых разрешений - возвращает код 200.
        """
    ...
    def test_detail_view(self):
        ...
        response = self.client.get(
            reverse("contracts:contracts-detail", kwargs={"pk": self.contract.pk})
        )
        self.assertEqual(response.status_code, 302)
        self.assertIn(str(settings.LOGIN_URL), response.url)
    ...
    def test_detail_manager(self):
        """
        Тест проверки возможности просмотра деталей контракта авторизованным
        пользователем. Без разрешений возвращает код 403, после их
        предоставления - 200.
        """
    ...
    def test_detail_admin(self):
        """
        Тест, подтверждающий возможность администратора просматривать детали
        контракта без предоставления дополнительных прав. Возвращает код 200.
        """
    ...
    def test_create_manager(self):
        """
        Тест проверки создания контракта авторизованным пользователем. Без наличия
        необходимых разрешений возвращается код 403, после их получения 302, создается
        контракт и идет перенаправлением на страницу списка контрактов.
        """
    ...
    def test_update_view(self):
        ...
        response = self.client.post(
            reverse("contracts:contract-update", kwargs={"pk": self.contract.pk}),
            data=data,
        )
        self.assertEqual(response.status_code, 302)
        self.assertIn(str(settings.LOGIN_URL), response.url)
    ...
    def test_update_manager(self):
        """
        Тест проверки возможности обновления контракта менеджером. Возвращает код
        403, после предоставления необходимых разрешений - 302 с обновлением полей
        контракта и перенаправлением на страницу деталей контракта.
        """
    ...
    def test_delete_contract(self):
        ...
        self.client.logout()
        response = self.client.post(
            reverse("contracts:contract-delete", kwargs={"pk": self.contract.pk})
        )
        self.assertEqual(response.status_code, 403)
    ...
    def test_delete_admin(self):
        """
        Тест проверки удаления контракта админом. Происходит удаление, возвращается
        код 302 и перенаправление на страницу списка контрактов.
        """
